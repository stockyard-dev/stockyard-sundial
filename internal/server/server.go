...
s.mux.HandleFunc("GET /api/time_entries",s.list);s.mux.HandleFunc("POST /api/time_entries",s.create);s.mux.HandleFunc("GET /api/time_entries/{id}",s.get);s.mux.HandleFunc("DELETE /api/time_entries/{id}",s.del)
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"time_entries":oe(s.db.List())})}
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.TimeEntry;json.NewDecoder(r.Body).Decode(&e);if e.Description==""{we(w,400,"description required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)stats(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]int{"time_entries":s.db.Count()})}
