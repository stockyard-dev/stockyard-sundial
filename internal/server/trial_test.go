...
func TestTrialMiddleware_writesBlocked(t *testing.T) {
	...
	cases := []struct {
		...
		{"PUT", "/api/time_entries/123"},
		{"DELETE", "/api/time_entries/123"},
		{"PUT", "/api/extras/time_entries/123"},
...
// newTestDB is a local helper since sundial has no existing test file
// that defines one. Opens a fresh SQLite store in a temp directory that
// t.Cleanup deletes after the test.
func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "sundial-trial-test-*")
	if err != nil {
